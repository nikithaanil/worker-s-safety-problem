...
def classify(df5):
    with open("./inputData/s_data1.csv", 'a') as f:
...
#     ls=[0,1,2]
#     # print (accuracy_score(0,predict))
#     y_pred = predict
#     #print(y_pred)
#     if y_pred == 0:
#         print("rest")
...
class MyoRaw(object):
    ...
    def on_emg(self, emg, moving):
        ...
        if emgvals!=None:
                ...
                cA_labelled = np.append([output],'rest')#change each time 
                cA_labelled = np.append([cA_labelled],'0')
...
if __name__ == '__main__':
    ...
    try:
        while datetime.datetime.now().second - start_time.second  <= 1:
            m.run()
            if HAVE_PYGAME:
